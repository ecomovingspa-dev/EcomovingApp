...
async function run() {
    console.log('=== INICIANDO RESTITUCIÓN DE FACTURA FOLIO 128 ===');
    ...
    if (vErr) {
        console.error('Error al buscar la factura Folio 128:', vErr);
        return;
    }
    ...
    // 2. Eliminar el abono ID 193 de la tabla abonos
    console.log('Eliminando Abono ID 193 (pago de factoring con error)...');
    ...
    console.log('Abono ID 193 eliminado correctamente.');

    // 3. Actualizar la factura en la tabla ventas
    console.log(`Actualizando factura Folio 128: saldo = $${Number(montoOriginal).toLocaleString('es-CL')}, estado_deuda = 'Vencida'...`);
    ...
    console.log('=== RESTITUCIÓN DE FOLIO 128 COMPLETADA SIN ERRORES ===');
}
